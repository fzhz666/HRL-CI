...
def train_meta_controller(run_name="Meta_step", episode_num=500,
    ...
    dir_name = 'meta_step_index_3_0.5'
    try:
    ...
    overall_episode = 0
    ita_per_episode = iteration_num_start  # 最大运行步数
    ...
    while True:
        ...
        while not done and ita_in_episode < ita_per_episode:
            ...
            # print('raw_goal, ctrl_steps = ', raw_goal, ctrl_steps)
            # print('ctrl_steps = ', ctrl_steps)

            goal_option = int(np.round(raw_goal * (traj_point_num - 1)))
            # print('goal_option= ', goal_option)

            ctrl_steps = raw_steps * 5 + 5  # ctrl_steps范围在5~15
            ctrl_steps = int(ctrl_steps)  # ctrl_steps范围在5~15
            # print('ctrl_steps = ', ctrl_steps)
            ...
            # print('目标点为=', env.target_path[ita_in_episode])
            # print('目标点1为=', env.target_path[ita_in_episode+1])
            # print('轨迹第一点为=', pred_target_traj[0])
            ...
            for i in range(1, len(target_traj_queue)):
                ...
                weight *= a  # 更新权重，叠加(1-a)的次方
            # 手动调整综合轨迹的起点为目标点的起点
            offset = next_pred_traj[0] - combined_pred_traj[0]
            combined_pred_traj += offset
