...
def train_meta_controller(run_name="Train_Meta_index_00", episode_num=500,
    ...
    while True:
        ...
        #     # 倒过来计算，所以第一个计算的就是最新的路径，后续路径乘以(1-a)的累计系数
        #     combined_pred_traj += weight * target_traj_queue[len(target_traj_queue)-i]
        #     weight *= a  # 更新权重，叠加(1-a)的次方

        # 加权移动平均对比实验修改
        # 假设你想要的权重数组是一个递减的权重，也可以设置为一样的权重（np.ones(n)）
        n = deque_number
        weights = np.linspace(1, 0, n + 1)  # 权重线性递减，从1到0
        # print("加权weights = ", weights)
        # 初始化加权移动平均的预测路径
        combined_pred_traj = np.zeros_like(pred_target_traj)
        # 计算加权移动平均
        for i in range(n):
            combined_pred_traj += weights[i] * target_traj_queue[n - i - 1]  # 使用倒序的路径和对应权重
        # 权重归一化，使得总权重为1
        combined_pred_traj /= np.sum(weights)
        ...
        # print("预测的更新时间步为ctrl_steps = ", ctrl_steps)
        ...
        while not done and ita_in_episode < ita_per_episode:
            ...
            if disappear_num % target_disappear_frequency == 0:
                # print("开始消失，ita_in_episode = ", ita_in_episode)
                # print("开始消失，start_idx = ", start_idx)
                # print("开始消失，observe_start_disappear = ", observe_start_disappear)
            ...
            if target_disappear_steps == 0:
                # print("不消失情况，ita_in_episode = ", ita_in_episode)
                observe_start = max(0, ita_in_episode - pred_tau + 1)
