...
from ddpg_networks_step import ActorNet, CriticNet
...
class MetaController:
    ...
    def __init__(self,
        ...
        '''
        self.actor_net = ActorNet(self.state_num + self.traj_feature, self.action_num, self.step_num,
                                  hidden1=actor_net_dim[0],
                                  hidden2=actor_net_dim[1],
                                  hidden3=actor_net_dim[2])
        self.critic_net = CriticNet(self.state_num + self.traj_feature, self.action_num, self.step_num,
                                    hidden1=critic_net_dim[0],
                                    hidden2=critic_net_dim[1],
                                    hidden3=critic_net_dim[2])
        self.target_actor_net = ActorNet(self.state_num + self.traj_feature, self.action_num, self.step_num,
                                         hidden1=actor_net_dim[0],
                                         hidden2=actor_net_dim[1],
                                         hidden3=actor_net_dim[2])
        self.target_critic_net = CriticNet(self.state_num + self.traj_feature, self.action_num, self.step_num,
                                           hidden1=critic_net_dim[0],
                                           hidden2=critic_net_dim[1],
                                           hidden3=critic_net_dim[2])
        ...
        # # 打印权重维度
        # print("fc3 weights dimensions:", fc3_weights.size())  # fc3 weights dimensions: torch.Size([1024, 1024])
        # fc4_weights = self.critic_net.fc4.weight
        # # 打印权重维度
        # print("fc4 weights dimensions:", fc4_weights.size())  # fc4 weights dimensions: torch.Size([1, 1024])
    ...
    def select_goal_step(self, joint_state_traj, explore=True, train=True):
        ...
        with torch.no_grad():
            joint_state_traj = np.array(joint_state_traj)
            joint_state_traj = torch.Tensor(joint_state_traj.reshape((1, -1))).to(self.device)
            # goal = self.actor_net(joint_state_traj).to('cpu')
            # step = self.actor_net(joint_state_traj).to('cpu')
            # 调用一次 actor_net
            out_action, out_step = self.actor_net(joint_state_traj)

            # 分别将每个张量移到 CPU
            goal = out_action.to('cpu')
            step = out_step.to('cpu')

            goal = goal.numpy().squeeze()
            step = step.numpy().squeeze()
        if train:
            if self.step_ita > self.epsilon_decay_start and self.epsilon > self.epsilon_end:
                if self.step_ita % self.epsilon_decay_step == 0:
                    self.epsilon = self.epsilon * self.epsilon_decay
            ...
            step = noise + step
            step = np.clip(step, 0., 1., )
        return np.float32(goal), np.float32(step)

    def remember(self, state, action, step, reward, next_state, done):
        """
        ...
        """
        # Unpack action_step tuple
        # action, step = action_step

        self.memory.append((state, action, step, reward, next_state, done))
    ...
    def replay(self):
        ...
        """
        state_batch, action_batch, step_batch, reward_batch, nx_state_batch, done_batch = _random_minibatch(self.memory,
                                                                                                     self.batch_size,
                                                                                                     self.device)
        ...
        '''
        with torch.no_grad():
            # 显式地将 nx_state_batch 转换为张量
            nx_state_batch = torch.as_tensor(nx_state_batch, device=self.device)

            # Unpack action_step_batch tuple
            # action_batch, step_batch = zip(*action_step_batch)
            action_batch = torch.as_tensor(action_batch, device=self.device)
            step_batch = torch.as_tensor(step_batch, device=self.device)

            nx_action_batch, nx_step_batch = self.target_actor_net(nx_state_batch)
            # 打印调试信息
            # print(f"Type of nx_state_batch: {type(nx_state_batch)}")
            # print(f"Type of nx_action_batch: {type(nx_action_batch)}")
            next_q = self.target_critic_net([nx_state_batch, nx_action_batch, nx_step_batch])
        ...
        current_q = self.critic_net([state_batch, action_batch, step_batch])
        ...
        current_action, current_step = self.actor_net(state_batch)
        actor_loss = -self.critic_net([state_batch, current_action, current_step])
...
def _random_minibatch(memory, batch_size, device):
    ...
    :return: state batch, action_step batch, reward batch, next state batch, done batch
    """
    state_batch, action_batch, step_batch, reward_batch, nx_state_batch, done_batch = zip(*random.sample(memory, batch_size))
    ...
    step_batch = torch.Tensor(step_batch).reshape(batch_size, -1).to(device)
    ...
    return state_batch, action_batch, step_batch, reward_batch, nx_state_batch, done_batch
