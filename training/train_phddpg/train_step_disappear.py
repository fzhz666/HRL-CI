...
sys.path.append('../../')
import training.train_phddpg.agent_step as agent
...
'''
# 输出预测点，有指数加权
def train_meta_controller(run_name="Meta_step", episode_num=500,
                          ...
                          state_num=20, traj_point_num=150, goal_feature=2, selection_num=1, step_num=1,
    ...
    dir_name = 'step_disappear_3_0.5'  # 尝试过5了
    try:
    ...
    meta_controller = agent.MetaController(state_num, selection_num, step_num, traj_point_num, goal_feature,
                                           reward_gamma=reward_gamma, memory_size=memory_size, batch_size=batch_size,
                                           epsilon_end=epsilon_end,
                                           epsilon_decay_start=epsilon_decay_start, epsilon_decay=env_epsilon_decay,
                                           epsilon_decay_step=epsilon_decay_step,
    ...
    # target_disappear_frequency = 20  # target_disappear_frequency次step循环内套target_disappear_duration次消失
    # disappear_flag = 0
    ...
    while True:
        ceshi = 0
        target_disappear_steps = 0
        target_disappear_duration = 30  # 目标消失的步数
        target_disappear_frequency = 100  # target_disappear_frequency次步内套target_disappear_duration次消失
        disappear_flag = 0
        observe_start_disappear = 0
        ...
        while not done and ita_in_episode < ita_per_episode:
            ...
            raw_goal, raw_steps = meta_controller.select_goal_step(joint_state_traj)

            goal_option = int(np.round(raw_goal * (traj_point_num - 1)))
            # print('goal_option= ', goal_option)

            ctrl_steps = raw_steps * 10   # ctrl_steps范围在0~10
            ctrl_steps = int(ctrl_steps)  # ctrl_steps范围在0~10
            ...
            while step < ctrl_steps and not done and ita_in_episode < ita_per_episode:
                ...
                if disappear_flag == 1 and target_disappear_steps == 0:
                    print('ceshi = ', ceshi)
                    break
            ...
            meta_controller.remember(joint_state_traj, raw_goal, raw_steps, meta_reward, next_joint_state_traj, done)
            ...
            # Save Model
            if (mate_overall_steps/10) % (save_steps / 4) == 0:  # 20次一个模型(/10)
                agent.save("../save_model_weights/" + dir_name, meta_controller.actor_net,
                           (mate_overall_steps/10) // (save_steps / 4), run_name)
